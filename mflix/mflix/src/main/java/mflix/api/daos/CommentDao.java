...
public class CommentDao extends AbstractMFlixDao {
    ...
    public boolean updateComment(String commentId, String text, String email) {
        ...
        // user own comments
        Bson queryFilter = Filters.and(Filters.eq("_id",new ObjectId(commentId)),
                                        Filters.eq("email",email));
        Comment comment = commentCollection.find(queryFilter).first();

        if(comment!=null){
            Bson update = set("text",text);
            UpdateResult updateResult = commentCollection.updateOne(queryFilter,update);
    ...
    public boolean deleteComment(String commentId, String email) {
        ...
        // TODO> Ticket Handling Errors - Implement a try catch block to
        // handle a potential write exception when given a wrong commentId.
        if (commentId.length() == 0 || email.length() == 0) {
            throw new IllegalArgumentException("commentId and email are both required.");
        }
        return false;
    }
